...
window.addEventListener('DOMContentLoaded', () => {
    ...
    $('loginForm').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // Clear the form fields so they're fresh next time.
            $('loginPassword').value = '';
        } catch (err) {
...
async function showMainApp() {
    ...
    // 🔒 Show/hide admin mode switch based on role
    $('modeSwitchWrapper').style.display = isAdmin ? 'flex' : 'none';

    // 🔒 HARD RESET all views first — this prevents "leftover" state
    $('studentLobbyView').classList.remove('active-view');
    $('studentQuizView').classList.remove('active-view');
    ...
    if (isAdmin) {
        ...
        // Student lands on student lobby — ALWAYS
